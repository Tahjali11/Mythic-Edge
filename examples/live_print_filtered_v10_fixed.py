...
OUT_FILENAME_PREFIX = os.environ.get("MYTHICEDGE_OUT_PREFIX", "mtga_filtered_events_v10")
...
def to_sheet_rows(event: Any) -> list[dict[str, Any]]:
    ...
    if kind == "GameResult":
        ...
        # Current game row: trust the top-level payload, not payload.results.
        ...
        # Match row: also trust top-level payload. payload.results has proven unreliable.
        if (
            match_id
            and match_id not in _MATCH_ROWS_POSTED
            and (
    ...
    if kind == "Rank":
        ...
        cl = payload.get("constructed_level", "")
        cp = payload.get("constructed_percentile")
        if cc == "Mythic" and cp not in (None, ""):
            rank_text = f"Mythic {cp}".strip()
        else:
...
def post_row_to_google_sheets(row: dict[str, Any]) -> None:
    ...
    try:
        ...
        print(f"POST ok: {row.get('event_family')} | {row.get('event_type')} | {row.get('scope')} | {row.get('match_id')} | {row.get('game_number')}")
