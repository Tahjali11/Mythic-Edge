...
OUT_FILENAME_PREFIX = os.environ.get("MYTHICEDGE_OUT_PREFIX", "mtga_filtered_events_v8")
...
# Local JSONL dedupe for turn markers.
...
_POSTED_SUBMIT_DECK_KEYS: set[tuple[Any, Any]] = set()
_POSTED_SIDEBOARD_KEYS: set[tuple[Any, Any]] = set()
_LAST_POSTED_RANK: str = ""
_POSTED_GAME_RESULT_KEYS: set[tuple[Any, Any, Any, Any, Any]] = set()
_POSTED_MATCH_RESULT_KEYS: set[tuple[Any, Any, Any]] = set()
...
def _update_context(event: Any) -> None:
    ...
    if kind == "MatchState":
        ...
        match_id, game_number, winning_team, _, _ = _extract_game_result_identity(payload)
        if match_id:
            _CONTEXT["current_match_id"] = match_id
        if game_number not in (None, ""):
            _CONTEXT["current_game_number"] = game_number
        if winning_team not in (None, "") and _CONTEXT["current_player_team"] == "":
            # Do not infer player team from winner_team; just keep current context untouched.
            pass
...
def _build_game_result_rows(event: Any) -> list[dict[str, Any]]:
    payload = getattr(event, "payload", {}) or {}
    ...
    # Emit exactly one game row for the current game result payload.
    if match_id and game_number not in (None, ""):
        game_key = (match_id, game_number, winning_team, result_type, reason)
        if game_key not in _POSTED_GAME_RESULT_KEYS:
            _POSTED_GAME_RESULT_KEYS.add(game_key)
            row = dict(base)
            row["scope"] = "Game"
    ...
    # Emit exactly one match row if the payload carries a match-scope result.
    match_result = None
    ...
    if match_result and match_id:
        match_winner = match_result.get("winningTeamId", winning_team)
        match_result_type = match_result.get("result", result_type)
        match_reason = match_result.get("reason", reason)
        match_key = (match_id, match_winner, match_result_type)
        if match_key not in _POSTED_MATCH_RESULT_KEYS:
            _POSTED_MATCH_RESULT_KEYS.add(match_key)
            row = dict(base)
            row["scope"] = "Match"
            row["match_id"] = match_id
            row["game_number"] = ""
            row["winner_team"] = match_winner
            row["result_type"] = match_result_type
            row["result_reason"] = match_reason
            rows.append(row)
...
def to_sheet_rows(event: Any) -> list[dict[str, Any]]:
    ...
    if kind == "GameResult":
        return _build_game_result_rows(event)
    ...
    if kind == "GameState":
        ...
        if not POST_GAMESTATE_ROWS:
            ...
            if turn_number != 1 or active_player in (None, ""):
                return []
        key = (
            match_id or _CONTEXT["current_match_id"],
            game_number or _CONTEXT["current_game_number"],
            turn_number or 1,
        )
    ...
    if kind == "Rank":
        ...
        if cp not in (None, "", 0):
            rank_text = f"{rank_text} {cp}".strip()
...
def post_row_to_google_sheets(row: dict[str, Any]) -> None:
    ...
    try:
        ...
        print(
            f"POST ok: {row.get('event_family')} | {row.get('scope')} | "
            f"{row.get('match_id')} | g={row.get('game_number')}"
        )
