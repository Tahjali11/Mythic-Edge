# Parser Corpus Connection Error Payload Coverage Contract

## Module

Connection error payload corpus coverage for the parser corpus parity report.

Plain English: this slice lets Mythic Edge cover exactly
`connection.connection_error_payload` with repo-owned synthetic metadata and
existing parser behavior evidence. It proves only that Mythic Edge has safe
corpus metadata for parser-owned `ConnectionError` payload handling. It does
not prove live reconnect behavior, disconnect behavior, firewall or network
drop behavior, private Player.log resilience, release readiness, full runtime
health, full connection parity, or full Mythic Edge corpus parity.

## Source Issue

- Issue: https://github.com/Tahjali11/Mythic-Edge/issues/364
- Previous child issue: https://github.com/Tahjali11/Mythic-Edge/issues/361
- Previous PR: https://github.com/Tahjali11/Mythic-Edge/pull/362
...
- observed_base_commit: `425b48b6937a95ef31f43435fb637a1b71cfe6b1`
- target_artifact:
...
`connection.connection_error_payload` scenario family. Parser modules and
events own the underlying connection-error interpretation. Corpus parity
...
Truth owner for `connection.connection_error_payload` coverage status:
...
- `connection_error.try_parse(...)` owns parser-owned `ConnectionError`
  emission for recognized markers and accepted headers.
- `ConnectionErrorEvent` owns the event kind and interactive-dispatch event
  identity.
- Router dispatch owns which log headers can reach the connection parser.
...
- Corpus coverage status is review metadata. It is not parser truth, runtime
  health truth, reconnect truth, disconnect truth, network reliability truth,
  firewall truth, diagnostics truth, workbook truth, analytics truth, AI truth,
...
```text
existing ConnectionError parser behavior and focused tests
  -> bounded synthetic committed corpus manifest/session-ledger metadata
  -> corpus parity coverage row for connection.connection_error_payload
```
...
- Corpus metadata must not turn a synthetic connection-error payload into a
  claim about live reconnects, disconnects, firewall drops, network outage
...
- reconnect, disconnect, firewall, network-drop, log-rotation, malformed-log,
...
- Tracker #158 is open.
- Issue #364 is open as the connection error payload corpus coverage child.
- The current corpus parity report returns
  `partial_coverage_map_ready (45 families, 6 committed, 25 missing)`.
- `connection.connection_error_payload` exists in the corpus taxonomy.
...
- `connection.disconnect` and `connection.firewall_or_network_drop` are
  taxonomy families but have no Mythic Edge-owned coverage entry.
- Sealed entry, sealed matches, and sealed deckbuild are already
  `covered_synthetic`; this issue must not reopen or change them.

Observed parser behavior:

- `src/mythic_edge_parser/parsers/connection_error.py` emits
  `ConnectionError` events for recognized Unity JSON error markers,
  connection-manager text markers, and matchmaking GRE connection-lost text.
- Unity JSON marker payloads emit `error_type` plus a nested `payload` mapping.
- Connection-manager reconnect-result markers emit `error_type` plus `result`
  when the result is one of the accepted current values.
- Connection-manager reconnect-outcome markers emit `error_type`, `outcome`,
  and `attempts`.
- Matchmaking GRE connection-lost text emits `error_type` only.
- Text-only connection error subtypes rely on event metadata raw bytes for raw
  evidence and do not expose an explicit `raw_*` payload field.
- Focused tests already cover accepted marker parsing, malformed JSON
  rejection, unknown result rejection, and unrelated-line rejection.
- `ConnectionErrorEvent` is an interactive-dispatch parser event kind.
- Router dispatch sends `CONNECTION_MANAGER` and `MATCHMAKING` entries to
  `connection_error`, and sends `UNITY_CROSS_THREAD_LOGGER` entries to
  `connection_error` after more specific parsers.

Observed prior audit guidance:

- The field-level parity audit classified `ConnectionError` as
  `documented_partial`.
- Parser-owned normalized fields include `error_type`, `result`, `outcome`,
  and `attempts` where present.
- Unity JSON errors preserve parsed payload evidence under `payload`.
- Text-only markers do not expose explicit raw text fields.
- No committed corpus coverage existed for connection errors before this
  issue.
...
```text
connection_error_payload_synthetic_v1
```
...
- `linked_issue`: `https://github.com/Tahjali11/Mythic-Edge/issues/364`
- `authorized_by_contract`:
  `docs/contracts/parser_corpus_connection_error_payload_coverage.md`
- `scenario_families`: `["connection.connection_error_payload"]`
...
- `match_shape`: `connection_error_payload_signal_only`
- `record_summary`: `synthetic_metadata_summary_only`
- `parser_coverage.event_families`: `{"ConnectionError": 1}` unless Codex C
  adds a reasoned contract-test note explaining a different safe count.
...
network trace, raw connection payload dump, host name, IP address, local path,
credential-like text, decklist, strategy note, or private report location.

### Allowed Evidence Model

Allowed committed evidence:

- existing parser source and focused parser tests for `ConnectionError`;
- existing event/schema snapshot evidence for `ConnectionError`;
...
- generic connection error type discriminator;
- generic parsed payload preservation under `payload` for Unity JSON markers;
- generic text-marker normalization for parser behavior awareness;
- generic missing-game-row statement because connection error payload coverage
...
- `connection.connection_error_payload` may become `covered_synthetic`.
...
- `connection.disconnect` must remain `missing` unless a later issue and
  contract authorize disconnect coverage.
- `connection.firewall_or_network_drop` must remain `missing` unless a later
  issue and contract authorize private/local report-only or synthetic coverage.

Because `connection_error.py` currently parses some reconnect text under the
`ConnectionError` event kind, Codex C must keep wording sharp: parser behavior
tests for reconnect text may be run as regression coverage, but this corpus
slice must not use those tests to claim `connection.reconnect` scenario
coverage.
...
The expected report row for `connection.connection_error_payload` should show:

- `coverage_status`: `covered_synthetic`
- `coverage_basis`: `["fixture_metadata_only", "parser_behavior_verified"]`
- `mythic_edge_entries`: `["connection_error_payload_synthetic_v1"]`
- notes stating that this proves synthetic parser-owned connection-error
  payload metadata only and does not prove reconnect, disconnect, network
  reliability, private smoke, release readiness, analytics truth, AI truth, or
  production behavior.
...
- `error_type` discriminator behavior;
- parsed Unity JSON error payload preservation under `payload`;
- text-marker normalization as observed parser behavior;
...
- Exactly one in-scope family is authorized:
  `connection.connection_error_payload`.
...
- `connection.disconnect` must not become covered from this issue.
...
- No new artifact may contain raw/private Player.log text, local absolute
  paths, credentials, webhook URLs, IP/network traces, decklists, strategy
  notes, generated data, SQLite files, runtime artifacts, or workbook exports.
...
- the manifest/schema cannot represent a safe synthetic connection-error entry
...
- coverage for reconnect, disconnect, firewall/network-drop, or runtime
  interruption appears necessary to satisfy the issue;
- private/live log evidence appears necessary;
- current branch state differs materially from
  `425b48b6937a95ef31f43435fb637a1b71cfe6b1` and changes the coverage counts
...
2. Compare `connection.connection_error_payload` current corpus status to this
   contract.
3. Confirm focused `ConnectionError` parser tests still pass without source
   changes.
4. Add the manifest entry for
   `connection_error_payload_synthetic_v1`.
5. Add the session-ledger entry for
   `connection_error_payload_synthetic_v1`.
6. Update `tests/test_corpus_parity_report.py` for the new row, summary counts,
   non-claims, and adjacent family statuses.
...
- Existing `ConnectionError` payload behavior remains unchanged.
...
- Whether a later connection-payload policy issue should add explicit `raw_*`
  fields for text-only `ConnectionError` subtypes.
- Whether future private/local smoke can safely produce report-only evidence
  for reconnect, disconnect, firewall, or network-drop families.
- Whether diagnostics should later summarize connection error payloads more
  richly.

Suspected gaps:

- Text-only `ConnectionError` subtypes do not expose explicit raw text fields.
- `TcpConnectionClose`, `WebSocketClosed`, and `MatchConnectionState` are
  adjacent connection/runtime events but are not covered by this corpus slice.
...
- `docs/contracts/parser_corpus_connection_error_payload_coverage.md` exists
  and names the owning layer, truth boundary, allowed evidence, forbidden
  evidence, protected surfaces, and validation plan.
- Codex C has a clear path to add one safe synthetic corpus entry:
  `connection_error_payload_synthetic_v1`.
- Only `connection.connection_error_payload` is authorized for status movement.
...
Act as Codex C: Module Implementer for issue #364, Connection error payload
corpus coverage path, under tracker #158.

Context:
- Tracker: https://github.com/Tahjali11/Mythic-Edge/issues/158
- Issue: https://github.com/Tahjali11/Mythic-Edge/issues/364
- Previous completed child issue: https://github.com/Tahjali11/Mythic-Edge/issues/361
- Previous PR: https://github.com/Tahjali11/Mythic-Edge/pull/362
- Previous merge commit on codex/parser-parity: 425b48b6937a95ef31f43435fb637a1b71cfe6b1
- Base branch: codex/parser-parity
- Contract: docs/contracts/parser_corpus_connection_error_payload_coverage.md
- Expected handoff: docs/implementation_handoffs/parser_corpus_connection_error_payload_coverage_comparison.md
- Expected report: docs/contract_test_reports/parser_corpus_connection_error_payload_coverage.md
...
needed to move `connection.connection_error_payload` to safe synthetic corpus
coverage without changing parser behavior or claiming adjacent connection
runtime coverage.
...
- Run the focused connection parser tests as evidence of existing parser-owned
  behavior.
- Write `docs/implementation_handoffs/parser_corpus_connection_error_payload_coverage_comparison.md`.
- Write `docs/contract_test_reports/parser_corpus_connection_error_payload_coverage.md`.
...
- Change parser behavior, router behavior, parser event classes, parser state
  ...
  deduplication.
- Move `connection.reconnect`, `connection.disconnect`, or
  `connection.firewall_or_network_drop` to covered status.
- Claim live reconnect, disconnect, firewall/network-drop, runtime
  interruption, private smoke, release readiness, full connection parity, or
...
- Commit private Player.log excerpts, private local logs, generated data,
  ...
  keys, webhook URLs, IP/network traces, decklists, strategy notes, or private
  reports.
...
workflow_handoff:
  ...
  verdict: "contract_ready_for_synthetic_connection_error_payload_coverage"
  risk_tier: "High"
  branch: "codex/parser-corpus-connection-error-payload-coverage"
  base_branch: "codex/parser-parity"
  validation:
  ...
  stop_conditions:
    ...
    - "Do not move connection.reconnect, connection.disconnect, or connection.firewall_or_network_drop to covered status."
    - "Do not claim live reconnect, disconnect, firewall/network-drop, runtime interruption, private smoke, release readiness, or full connection parity."
    - "Do not import, copy, mirror, or commit Manasight raw logs or external corpus contents."
    - "Do not commit private Player.log excerpts, private local logs, generated data, SQLite files, runtime artifacts, workbook exports, credentials, tokens, API keys, webhook URLs, IP/network traces, decklists, strategy notes, or private reports."
```
