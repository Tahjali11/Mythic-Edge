...
1. Resolve the repo root before reading or editing files.
   - Preferred path on this Windows desktop: `C:\Users\Tahj Blow\Desktop\MTG Resources\MythicEdge`
   - If the session starts from `C:\Users\Tahj Blow\Desktop\MTG Resources`, switch into `MythicEdge`.
   - On other machines, find the clone root by locating `AGENTS.md` and `pyproject.toml`.
...
- avoid raw logs, generated data, secrets, runtime status, failed posts, workbook exports, and local-only artifacts
- link the issue and tracker
- report target branch, validation, and remaining risk

Prefer `C:\Program Files\GitHub CLI\gh.exe` if `gh` is not available on PATH.

## Installing This Repo-Owned Skill
