...
def test_opponent_card_observations_still_remain_deferred() -> None:
    connection = _connect()
    replay = _with_actions(_action_entry())
    replay["opponent_card_observations"] = [{"match_id": "match:test:gameplay"}]

    result = ingest_parser_normalized_replay(connection, replay, started_at="now", finished_at="done")

    assert result.skipped == {"opponent_card_observations": 1}
    assert result.warnings == ["opponent_card_observations are accepted but deferred by the first ingest pass"]
    assert _count(connection, "gameplay_actions") == 1
